/*! ui-grid - v2.0.7-g82d25ed - 2014-04-06
  ...
  angular.module('ui.grid').directive('uiGridBody', ['$log', '$document', '$timeout', 'uiGridConstants', 'gridUtil',
    ...
    return {
      ...
      link: function($scope, $elm, $attrs, uiGridCtrl) {
        ...
        // TODO Positive ??!!
        $scope.toggleSelected = function (event, index) {

          var row = $scope.grid.renderedRows[index];

          $scope.idSelected = row.index;

          if ($scope.grid.options.selectedRow) {
              $scope.grid.options.selectedRow(row.entity);
          }
        };

        $(window).on('resize', function (events, layout) {
            var sumColWidth = uiGridCtrl.grid.options.colMinWidth * uiGridCtrl.grid.columns.length;
            uiGridCtrl.grid.gridWidth = $(this).innerWidth();

            
            if (!layout && uiGridCtrl.grid.gridWidth > sumColWidth) {
              uiGridCtrl.refreshCanvas(true);
            } else {
              uiGridCtrl.grid.styleComputations
                .forEach(function (compInfo) {
                  compInfo.func.call(uiGridCtrl.grid, $scope);  
                    if (compInfo.func.name == 'updateHorizontalScrollbar') {
                      compInfo.func.call(uiGridCtrl.grid, $scope);  
                    }
                });
            }
          ...
          post: function ($scope, $elm, $attrs, uiGridCtrl) {
            ...
            function updateColumnWidths() {
              ...
              uiGridCtrl.grid.columns.forEach(function (column) {
                ret = ret + ' .grid' + uiGridCtrl.grid.id + ' .col' + column.index + ' { width: ' + column.width + 'px; min-width: 200px; }';
                sumColWidth += column.width;
              });

              if (uiGridCtrl.grid.getViewportWidth() >= sumColWidth || uiGridCtrl.grid.columns[0].width > uiGridCtrl.grid.columns[0].minWidth) {
                var colWidth = uiGridCtrl.grid.getViewportWidth() / uiGridCtrl.grid.columns.length;
                ret = '';
                uiGridCtrl.grid.columns.forEach(function (column) {
    ...
    return {
      ...
      link: function($scope, $elm, $attrs, uiGridCtrl) {
        ...
        function updateHorizontalScrollbar(gridScope) {
          ...
          //console.log('Hor scrollbar: ' + scrollbarWidth);
          ...
          if ($scope.type === 'vertical') {
            ...
            scrollPercentage = args.y.percentage;
              if (scrollPercentage >= 1) {
                  ...
                  scrollPercentage = 1;
                  // TODO Positive ??!!
                  $scope.$emit('uiGridScrollEndList', {});
              }
          }
          else if ($scope.type === 'horizontal') {
...
//        }
      ...
      Grid.prototype.addRows = function(newRawData) {
        var self = this,
            i = 0,
            newRawDataLength = newRawData.length;

          // TODO Positive ??!!
        var lastIndex = 0;
          if (self.rows.length > 0) {
              lastIndex = self.rows[self.rows.length-1].index + 1;
              newRawData[i].index = lastIndex;
          }

        for (i; i < newRawDataLength; i++) {
          self.rows.push( self.processRowBuilders(new GridRow(newRawData[i], lastIndex++)) );
        }
      };
...
//           ret = this.options.colMinWidth * this.columns.length;
        }
        ...
        ret = this.columns[0].width * this.columns.length;
        
        //console.log('CanvasWidth :' + ret);
        return ret;
      };
      ...
      link: function ($scope, $elm, $attrs, uiGridCtrl) {
        ...
        $elm.on('dblclick', function() {
          ...
          Array.prototype.forEach.call(cells, function (cell) {
              // Get the cell width
              $log.debug('Col width', gridUtil.elementWidth(cell));
...
angular.module('ui.grid').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('ui-grid/ui-grid-row',
    "<div><div ng-repeat=\"col in grid.renderedColumns track by $index\" class=\"ui-grid-cell col{{ col.index }}\">" +
        "<div class=\"ui-grid-vertical-bar\">&nbsp;</div>" +
        "<div ui-grid-cell=\"\" col=\"col\" row=\"row\" row-index=\"row.index\" col-index=\"col.colDef.index\"></div>" +
    "</div>"
  );
